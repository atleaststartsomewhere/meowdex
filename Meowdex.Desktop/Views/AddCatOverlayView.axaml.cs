...
public sealed partial class AddCatOverlayView : UserControl
{
    public AddCatOverlayView()
    {
        InitializeComponent();
    ...
    {
        if (DataContext is not AddCatOverlayViewModel vm)
        {
            return;
        }
    ...
    private bool HandleTabCycle(KeyEventArgs e)
    {
        ...
        if (currentIndex < 0)
        {
            int startIndex = tabOrder.IndexOf(NameInput);
            if (startIndex < 0)
            {
                startIndex = 0;
            }

            nextIndex = shiftPressed
                ? (startIndex - 1 + tabOrder.Count) % tabOrder.Count
                : startIndex;
        }
