...
public sealed partial class AvailableSectionView : UserControl
{
    private bool _openingEdit;
    private CancellationTokenSource? _teamToastCts;

    public AvailableSectionView()
    {
        InitializeComponent();
    ...
    private async void OnGridRowPointerPressed(object? sender, PointerPressedEventArgs e)
    {
        ...
        {
            Trace.WriteLine($"AvailableSectionView row edit workflow failed: {ex}");
        }
        finally
        {
            _openingEdit = false;
    ...
    private void ShowTeamToast(string message)
    {
        ...
        var toastText = this.FindControl<TextBlock>("TeamToastText");
        var toast = this.FindControl<Border>("TeamToast");
        if (toastText is null || toast is null)
        {
            return;
        }
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                await Dispatcher.UIThread.InvokeAsync(() => toast.IsVisible = false);
            }
